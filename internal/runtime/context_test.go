...
func TestSetterWhenProviderNotExists(t *testing.T) {
	themeCtx := CreateContext("light")
	...
	delete(parent.Providers, themeCtx.id)

	setter("blue")

	if len(sess.DirtyQueue) != 0 {
		t.Error("expected no dirty components when provider is missing")
	}
}

func TestSetterTypeAssertionFailure(t *testing.T) {
	themeCtx := CreateContext("light")
	...
	parent.Providers[themeCtx.id] = 123

	setter("blue")

	if parent.Providers[themeCtx.id] != "blue" {
		t.Errorf("expected setter to update value to 'blue', got %v", parent.Providers[themeCtx.id])
	}

	if len(sess.DirtyQueue) != 1 {
		t.Error("expected parent to be marked dirty after type mismatch")
	}
}

func TestRecordContextDepNilInstance(t *testing.T) {
	var inst *Instance
	inst.recordContextDep(contextID(1))
}

func TestRecordContextDepCreatesMap(t *testing.T) {
	inst := &Instance{
		ID:        "test",
		HookFrame: []HookSlot{},
	}
	...
	if inst.ContextDeps != nil {
		t.Error("expected ContextDeps to be nil initially")
	}

	inst.recordContextDep(contextID(1))

	if inst.ContextDeps == nil {
		t.Error("expected ContextDeps to be created")
	}

	if _, ok := inst.ContextDeps[contextID(1)]; !ok {
		t.Error("expected context dependency to be recorded")
	}
}
