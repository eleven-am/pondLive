...
func UseParams(ctx *runtime2.Ctx) map[string]string {
...
func UseParam(ctx *runtime2.Ctx, key string) string {
...
// Returns nil if no router provider is present.
func UseQuery(ctx *runtime2.Ctx) url.Values {
	loc := UseLocation(ctx)
	if loc == nil {
		return nil
	}
...
func UseSearchParam(ctx *runtime2.Ctx, key string) (func() []string, func([]string)) {
	loc := UseLocation(ctx)
...
// The setter triggers navigation to update the URL.
func UseSearchParams(ctx *runtime2.Ctx) (func() url.Values, func(url.Values)) {
	loc := UseLocation(ctx)
