...
func TestDiffStylesheetNilMaps(t *testing.T) {
	...
	next.Stylesheet = &metadata.Stylesheet{
		Rules: []metadata.StyleRule{
			{Selector: "card", Decls: []metadata.Declaration{{Property: "color", Value: "blue"}}},
		},
	}
	patches := Diff(prev, next)
...
func TestDiffIndexedChildrenShiftOnInsert(t *testing.T) {
	...
	if addCount != 2 {
		t.Fatalf("expected 2 adds for the new div elements, got %d adds", addCount)
	}
}
...
func TestDiffIndexedInsertInMiddle(t *testing.T) {
	prev := withChildren(elementNode("div"),
		withKey(elementNode("header"), "header"),
		withKey(elementNode("footer"), "footer"),
	)

	next := withChildren(elementNode("div"),
		withKey(elementNode("header"), "header"),
		withKey(elementNode("main"), "main"),
		withKey(elementNode("footer"), "footer"),
	)
...
func TestDiffIndexedSimplePositionMatching(t *testing.T) {
	prev := withChildren(elementNode("ul"),
...
func TestDiffStylesheetModification(t *testing.T) {
	...
	prev.Stylesheet = &metadata.Stylesheet{
		Rules: []metadata.StyleRule{
			{Selector: ".btn", Decls: []metadata.Declaration{{Property: "color", Value: "red"}, {Property: "padding", Value: "10px"}}},
		},
	}
	...
	next.Stylesheet = &metadata.Stylesheet{
		Rules: []metadata.StyleRule{
			{Selector: ".btn", Decls: []metadata.Declaration{{Property: "color", Value: "blue"}, {Property: "margin", Value: "5px"}}},
		},
	}
...
func TestDiffStylesheetRemoval(t *testing.T) {
	...
	prev.Stylesheet = &metadata.Stylesheet{
		Rules: []metadata.StyleRule{
			{Selector: ".card", Decls: []metadata.Declaration{{Property: "color", Value: "blue"}, {Property: "padding", Value: "1rem"}}},
		},
	}
		...
		MediaBlocks: []metadata.MediaBlock{
			{
				...
				Rules: []metadata.StyleRule{
					{Selector: ".container", Decls: []metadata.Declaration{{Property: "width", Value: "750px"}}},
				},
			},
		},
	}
		...
		MediaBlocks: []metadata.MediaBlock{
			{
				Query: "(min-width: 768px)",
				Rules: []metadata.StyleRule{
					{Selector: ".container", Decls: []metadata.Declaration{{Property: "width", Value: "720px"}}},
				},
			},
		},
	}
