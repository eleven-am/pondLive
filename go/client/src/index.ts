...
class LiveUI extends EventEmitter<LiveUIEvents> {
    private options: Required<Omit<LiveUIOptions, 'boot'>> & { boot?: BootPayload };
    ...
    // Reactive state
    private connectionState = new Signal<ConnectionState>({status: 'disconnected'});
    ...
    private readonly handleDebugKeydown = (event: KeyboardEvent) => this.onDebugKeydown(event);
    ...
    async connect(): Promise<void> {
        ...
        try {
            ...
            if (typeof document !== 'undefined') {
                registerNavigationHandler((path, query, hash) => this.sendNavigation(path, query, hash));
            }
            ...
            this.channel = this.client.createChannel<LiveUIEventMap>(topic, joinPayload);
     ...
     */
    sendEventDebounced(event: { hid: string; payload: EventPayload }, delay: number = 300): void {
            ...
            try {
                ...
                switch (effectType) {
                    ...
                    case 'replace':
                        ...
                        break;
                    case 'dispatch':
                        this.dispatchCustomEvent((effect as DispatchEffect).eventName, (effect as DispatchEffect).detail);
                        break;
                    case 'metadata':
    ...
    private applyMetadataEffect(effect: MetadataEffect): void {
        ...
        if (Object.prototype.hasOwnProperty.call(effect, 'title') && effect.title !== undefined) {
        ...
        const upsertMeta = (payload: MetadataTagPayload, typeValue: string) => {
            ...
            let element = head.querySelector(`meta[${keyAttr}="${payload.key}"]`) as HTMLMetaElement | null;
            if (!element) {
            ...
            if (payload.name !== undefined) element.setAttribute('name', payload.name);
            if (payload.content !== undefined) element.setAttribute('content', payload.content);
            if (payload.property !== undefined) element.setAttribute('property', payload.property);
            if (payload.charset !== undefined) element.setAttribute('charset', payload.charset);
            if (payload.httpEquiv !== undefined) element.setAttribute('http-equiv', payload.httpEquiv);
            if (payload.itemProp !== undefined) element.setAttribute('itemprop', payload.itemProp);
            if (payload.attrs) {
        ...
        const upsertLink = (payload: MetadataLinkPayload) => {
            ...
            let element = head.querySelector(`link[${keyAttr}="${payload.key}"]`) as HTMLLinkElement | null;
            if (!element) {
            ...
            if (payload.href !== undefined) element.setAttribute('href', payload.href);
            if (payload.type !== undefined) element.setAttribute('type', payload.type);
            if (payload.as !== undefined) element.setAttribute('as', payload.as);
            if (payload.media !== undefined) element.setAttribute('media', payload.media);
            if (payload.hreflang !== undefined) element.setAttribute('hreflang', payload.hreflang);
            if (payload.title !== undefined) element.setAttribute('title', payload.title);
            if (payload.crossorigin !== undefined) element.setAttribute('crossorigin', payload.crossorigin);
            if (payload.integrity !== undefined) element.setAttribute('integrity', payload.integrity);
            if (payload.referrerpolicy !== undefined) element.setAttribute('referrerpolicy', payload.referrerpolicy);
            if (payload.sizes !== undefined) element.setAttribute('sizes', payload.sizes);
            if (payload.attrs) {
                for (const [k, v] of Object.entries(payload.attrs)) {
                    if (v != null) {
                        element.setAttribute(k, String(v));
        ...
        const upsertScript = (payload: MetadataScriptPayload) => {
            ...
            let element = head.querySelector(`script[${keyAttr}="${payload.key}"]`) as HTMLScriptElement | null;
            if (!element) {
            ...
            if (payload.integrity !== undefined) element.setAttribute('integrity', payload.integrity);
            if (payload.referrerpolicy !== undefined) element.setAttribute('referrerpolicy', payload.referrerpolicy);
            if (payload.nonce !== undefined) element.setAttribute('nonce', payload.nonce);
            if (payload.attrs) {
                for (const [k, v] of Object.entries(payload.attrs)) {
                    if (v != null) {
                        element.setAttribute(k, String(v));
        ...
        if (Object.prototype.hasOwnProperty.call(effect, 'description') && effect.description !== undefined) {
            const content = effect.description;
            if (content.trim().length > 0) {
                upsertMeta({key: 'description', name: 'description', content}, 'description');
            } else {
                removeByKeys(['description']);
            }
        }
    ...
    private hasStateChanged(oldState: ConnectionState, newState: ConnectionState): boolean {
    ...
    private recordDiagnostic(msg: ErrorMessage): void {
        ...
        const existingIndex = this.diagnostics.findIndex((item) => item.key === key);
        if (existingIndex >= 0) {
    ...
    private buildDiagnosticKey(msg: ErrorMessage): string {
        ...
        return [msg.code ?? '', msg.message ?? '', component, hook, phase, panic].join('|');
    }
    ...
    private joinPubsubTopic(topic: string): void {
        ...
        const channel = this.client.createChannel<LiveUIEventMap>(`pubsub/${topic}`, {
    ...
    private renderErrorOverlay(): void {
        ...
        if (this.diagnostics.length === 0) {
            ...
            const ordered = [...this.diagnostics].sort((a, b) => b.timestamp - a.timestamp);
    ...
    private createDiagnosticElement(entry: RuntimeDiagnosticEntry): HTMLElement {
        ...
        const componentLabel = details?.componentName && details.componentId
    ...
    private appendMeta(container: HTMLElement, label: string, value?: string | null): void {
        if (!value) {
            return;
        }
