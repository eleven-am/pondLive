...
// Example:
//
//	var optionalRef *dom.ElementRef[HTMLDivElement]
...
// Example:
//
//	buttonRef := dom.NewElementRef("btn-1", HTMLButtonElement{})
//	buttonRef.On("customEvent", func(evt dom.Event) dom.Updates {
...
// Both raw ElementRef[T] and wrapper refs implement this interface.
type Attachment interface {
	AttachTo(*StructuredNode)
}

// AttachTo implements the Attachment interface for ElementRef[T].
func (r *ElementRef[T]) AttachTo(node *StructuredNode) {
	AttachElementRef(r, node)
}
...
func AttachElementRef[T ElementDescriptor](ref *ElementRef[T], node *StructuredNode) {
	...
	if ref.attached {
		panic("dom: element ref attached multiple times")
	}
	if node.Descriptor == nil {
		panic(fmt.Sprintf("dom: element <%s> missing descriptor; regenerate builders", node.Tag))
	}
	if _, ok := any(node.Descriptor).(T); !ok {
		panic(fmt.Sprintf("dom: cannot attach ref for %T to <%s>", ref.descriptor, node.Tag))
	}
	ref.attached = true
	if node.Events == nil {
