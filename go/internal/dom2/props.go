...
type attachProp[T ElementDescriptor] struct {
	ref *ElementRef[T]
}

func (p attachProp[T]) isProp() {}

func (p attachProp[T]) ApplyTo(n *StructuredNode) {
	AttachElementRef(p.ref, n)
}

// Attach binds an element ref to the element
func Attach[T ElementDescriptor](ref *ElementRef[T]) Prop {
	return attachProp[T]{ref: ref}
}
