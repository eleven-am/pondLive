...
type resolvableNode interface {
	dom.Node
	resolve(runtime.Ctx, *RouterStore) dom.Node
}

// ResolveTree walks the provided DOM node and resolves any router placeholders
// using the supplied store.
func ResolveTree(ctx runtime.Ctx, node dom.Node, store *RouterStore) dom.Node {
	...
	switch v := node.(type) {
	case resolvableNode:
		return ResolveTree(ctx, v.resolve(ctx, store), store)
	case *dom.Element:
		clone := *v
		if len(v.Children) > 0 {
			clone.Children = make([]dom.Node, len(v.Children))
			for i, child := range v.Children {
				clone.Children[i] = ResolveTree(ctx, child, store)
			}
		}
		return &clone
	case *dom.FragmentNode:
		...
		for i, child := range v.Children {
			children[i] = ResolveTree(ctx, child, store)
		}
		return &dom.FragmentNode{Children: children}
	case *dom.ComponentNode:
		clone := *v
		if v.Child != nil {
			clone.Child = ResolveTree(ctx, v.Child, store)
		}
		return &clone
