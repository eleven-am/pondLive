...
func NewUploadHandler(reg *SessionRegistry) *UploadHandler {
...
func (h *UploadHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if h == nil || h.registry == nil {
		http.Error(w, "server: upload handler not available", http.StatusServiceUnavailable)
		return
	}
	...
	if sid == "" || uploadID == "" {
		http.Error(w, "server: invalid upload target", http.StatusBadRequest)
		return
	}
	...
	if component == nil {
		http.Error(w, "server: session unavailable", http.StatusGone)
		return
	}
	...
	if err != nil {
		http.Error(w, "server: expected multipart form", http.StatusBadRequest)
		return
	}
	...
	if stageErr != nil {
		...
		http.Error(w, "server: failed to process upload", status)
		return
	}
	...
	if staged.Reader == nil {
		...
		_ = sess.Flush()
		cleanupUploadedFile(staged)
		http.Error(w, "server: missing file", http.StatusBadRequest)
		return
	}
	...
	if err := sess.Flush(); err != nil {
		cleanupUploadedFile(staged)
		http.Error(w, "server: failed to flush upload", http.StatusInternalServerError)
		return
	}
